...
public class EventDao extends Dao {
    ...
    public Event getPendingEvent(String packageName) {
        Cursor cursor = query(packageName, true);
        if (cursor.moveToNext()) {
            return getObject(cursor, getColumnIndexes(cursor));
        }
        cursor.close();
        return null;
    }

    public void confirmPendingEvent(Event event) {
        String whereClause = KEY_PENDING + "=1 AND " + KEY_TIME + "=? AND " + KEY_PACKAGE_NAME + "=?";
        String[] whereArgs = new String[] {Long.toString(event.getTime()), event.getPackageName()};
        ContentValues contentValues = new ContentValues();
        contentValues.put(KEY_PENDING, 0);
    ...
    public void insert(Event event) {
        ...
        contentValues.put(KEY_PENDING, event.isPending() ? 1 : 0);
        db.insert(TABLE, null, contentValues);
    }

    public void cleanupUpdates(String packageName) {
        db.delete(TABLE, KEY_PACKAGE_NAME + "=? AND (" + KEY_TYPE + "=? OR " + KEY_TYPE + "=?) AND " + KEY_TIME + ">" + KEY_TIME + "-2000", new String[] {packageName, Event.TYPE.INSTALLATION.name(), Event.TYPE.REMOVAL.name()});
    }

    private Cursor query(String packageName) {
        return query(packageName, false);
    }

    private Cursor query(String packageName, boolean pending) {
        String selection = KEY_PENDING + "=?";
        String[] selectionArgs = new String[] {pending ? "1" : "0"};
        if (!TextUtils.isEmpty(packageName)) {
            selection = KEY_PACKAGE_NAME + "=? AND " + selection;
            selectionArgs = new String[] {packageName, selectionArgs[0]};
        }
        String limitString = "";
        if (pending) {
            limitString = "1";
        }
        return db.query(TABLE, null, selection, selectionArgs, null, null, KEY_TIME + " DESC", limitString);
    }
    ...
    private boolean sameChangesAsPrevious(String packageName, String changes) {
        Cursor cursor = db.query(
            TABLE,
            null,
            KEY_PENDING + "=? AND " + KEY_PACKAGE_NAME + "=? AND (" + KEY_TYPE + "=? OR " + KEY_TYPE + "=?) AND LENGTH(" + KEY_CHANGES + ")>0",
            new String[] {"0", packageName, Event.TYPE.INSTALLATION.name(), Event.TYPE.UPDATE.name()},
            null,
            null,
            KEY_TIME + " DESC",
            "1"
    ...
    private Event getObject(Cursor cursor, Map<String, Integer> columnIndexes) {
        ...
        item.setPending(cursor.getInt(columnIndexes.get(KEY_PENDING)) == 1);
        return item;
    }
    ...
    private Map<String, Integer> getColumnIndexes(Cursor cursor) {
        ...
        columnIndexes.put(KEY_PENDING, cursor.getColumnIndex(KEY_PENDING));
